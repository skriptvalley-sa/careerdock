  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[var(--color-text)]">My Lists</h1>
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-full max-w-md rounded-lg bg-card p-6 shadow-xl">
            <h2 className="text-lg font-semibold text-[var(--color-text)]">Create new list</h2>
            <form onSubmit={handleCreate} className="mt-4 space-y-4">
              <div>
                <label htmlFor="list-name" className="block text-sm font-medium text-[var(--color-text)]">
                  Name
                </label>
                <input
                  id="list-name"
              ...
              <div>
                <label htmlFor="list-desc" className="block text-sm font-medium text-[var(--color-text)]">
      ...
      {!lists || lists.length === 0 ? (
        <div className="mt-8 rounded-lg border border-dashed border-edge p-12 text-center">
          <p className="text-sm text-[var(--color-text-muted)]">No lists yet. Create one to start tracking applications.</p>
        </div>
      ) : (
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {lists.map((list) => (
            <div
              key={list.id}
              className="group relative card-hover rounded-lg border border-edge bg-card p-5 transition-all"
            >
              <Link href={`/lists/${list.id}`} className="block">
                <h3 className="text-lg font-semibold text-[var(--color-text)]">{list.name}</h3>
                {list.description && (
                  <p className="mt-1 line-clamp-2 text-sm text-[var(--color-text-muted)]">{list.description}</p>
                )}
              ...
              <button
                ...
                className="absolute right-3 top-3 hidden rounded p-1 text-[var(--color-text-muted)] hover:bg-red-900/30 hover:text-red-400 group-hover:block"
                title="Delete list"
              >
